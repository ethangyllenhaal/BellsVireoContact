...
# One is a VCF file with all relevant samples included.
...
# Future implementations would also take the population directory as input. 
#
...
for pop1 in `ls pops_Z`; do
	...
	for pop2 in `ls pops_Z`; do
		...
		then
			...
			vcftools --vcf $1 --weir-fst-pop pops_Z/$pop1 --weir-fst-pop pops_Z/$pop2 --out fst_output
